...
test.describe('Landing page as anonymous user', () => {
  test('deep link redirects to landing page with login link carrying the deep link', async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole('heading', { name: 'Tervetuloa Ludos-palveluun' }),
    ).toBeVisible()
  ...
  test('front page redirects to landing page without a deep link', async ({
    page,
  }) => {
    await page.goto('/')
  ...
  test('language can be changed to Swedish and back', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: 'Välkommen till Ludos-tjänsten' }),
    ).toBeVisible()
...
test.describe('Landing page as logged in user', () => {
  ...
  test('login does not redirect off-site', async ({ page }) => {
    const response = await page.request.get(
      `/api/auth/login?to=${encodeURIComponent('//evil.com')}`,
      {
        maxRedirects: 0,
      },
    )
