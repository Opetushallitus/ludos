...
async function updateInstruction(
  ...
  if (exam === Exam.LD) {
    await expect(instructionCard.getByTestId('instruction-title')).toHaveText(
      await koodiLabel(KoodistoName.LUDOS_LUKIODIPLOMI_AINE, '9')
    )
  } else {
    await expect(instructionCard.getByTestId('instruction-title')).toHaveText(previousName)
  }
