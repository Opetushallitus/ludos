...
import { AssignmentIn, AssignmentState } from '../../types'
...
export const AssignmentEditForm = ({ header, description }: { header: string; description: string }) => {
  const navigate = useNavigate()
  const { pathname, state } = useLocation()
  const match = useMatch(`/${contentKey}/:examType/:assignmentType/update`)

  const { examType } = match!.params
  const assignment = (state?.assignment as AssignmentIn) || null

  const defVals = (): SukoAssignmentForm | undefined => {
  ...
  const {
    ...
    control,
    formState: { errors }
  } = useForm<SukoAssignmentForm>({ mode: 'onBlur', resolver: zodResolver(schema), defaultValues: defVals() })
